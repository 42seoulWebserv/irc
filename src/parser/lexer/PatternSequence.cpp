...
PatternSequence::PatternSequence(const PatternSequence &rhs) : Pattern(rhs), patterns_(rhs.patterns_)
{
}
...
Pattern *PatternSequence::clone() const
{
    return new PatternSequence(*this);
}

bool PatternSequence::compile(std::stringstream &ss, ParseResult &res, bool debug) const
{
    ...
    for (size_t i = 0; i < patterns_.size(); i++) {
        ...
        else if (debug) this->debug(result.getDepth(), pattern.getName() + " matched");
    }
