...
const std::string &Pattern::getAlias() const
{
    return alias_;
}
...
{
    std::cout << std::string(depth * 2, ' ') << getName() << ": " << message << std::endl;
}

void Pattern::setParser(Parser *parser)
{
    parser_ = parser;
}
...
bool Pattern::isFlatResult() const
{
    return isFlatResult_;
}
...
bool Pattern::isIgnoreResult() const
{
    return isIgnoreResult_;
}
