...
ParseResult::ParseResult(const std::string &name, int depth, bool isFlat, bool isIgnore) :
    name_(name),
    values_(),
    children_(),
    depth_(depth),
...
void ParseResult::setName(const std::string &name)
{
    name_ = name;
}

const std::string &ParseResult::getName() const
{
    return name_;
}
...
int ParseResult::getDepth() const
{
    return depth_;
}

void ParseResult::push(const ParseResult &child)
{
...
void ParseResult::insertFlatValues(std::vector<std::string> &values, const ParseResult &target)
{
