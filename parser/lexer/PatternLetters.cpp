...
Pattern *PatternLetters::clone() const
{
    return new PatternLetters(*this);
}

bool PatternLetters::compile(std::stringstream &ss, ParseResult &res, bool debug) const
{
    ...
    while (ss.eof() == false) {
        ...
        if (std::isspace(c)) { ss.unget(); break; }

        size_t pos = letters_.find(c);
        if (ignore_ == true && pos != std::string::npos) { ss.unget(); break; }
        if (ignore_ == false && pos == std::string::npos) { ss.unget(); break; }
        str.push_back(c);
    }
    if (str.empty()) {
        ss.seekg(begin);
